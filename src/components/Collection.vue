      ...
      <div class="error h-screen">
        <div class="text-center max-w-md">
          ...
          <p class="text-neutral-400 dark:text-neutral-500 mb-6">
            Either your settings for the
            <code class="text-sm bg-neutral-100 dark:bg-neutral-850 rounded-lg p-1">{{
              schema.name
            }}</code>
            collection are wrong or you may need to create the
            <code class="text-sm bg-neutral-100 dark:bg-neutral-850 rounded-lg p-1">{{
              schema.path
            }}</code>
    ...
    <template v-else-if="collection">
      <div class="max-w-screen-xl mx-auto p-4 lg:p-8">
        ...
        <header class="flex gap-x-2 mb-8 items-center">
          <h1 class="font-semibold tracking-tight text-2xl lg:text-4xl mr-auto">
            {{ schema.label || schema.name }}
          </h1>
          <Dropdown :dropdownClass="'!max-w-none w-52 !z-[21]'" v-if="schema.subfolders !== false">
            <template #trigger>
              <button
                class="btn-icon group-[.dropdown-active]:bg-neutral-100 dark:group-[.dropdown-active]:bg-neutral-850"
              >
            ...
            <template #content>
              <ul>
                <li>
                  <a
                    class="link w-full"
                    :href="`https://github.com/${props.owner}/${props.repo}/blob/${props.branch}/${folder}`"
                    target="_blank"
                  >
                ...
                <li>
                  <button class="link w-full" @click="openAddFolderModal()">Add a folder</button>
                </li>
              </ul>
            </template>
          </Dropdown>
          <router-link :to="{ name: 'new', query: { folder: folder } }" class="btn-primary"
            >Add an entry</router-link
          >
        </header>
        ...
        <div class="flex justify-between items-center mb-4 flex-row gap-x-2">
          <div class="relative w-full">
            <input
              ...
              class="w-full !pl-9 placeholder-neutral-400 dark:placeholder-neutral-500"
            />
          ...
          <Dropdown :elementClass="'z-30'" :dropdownClass="'!max-w-none'">
            ...
            <template #content>
              <ul class="sticky">
                <li>
                  <div class="font-medium text-xs pb-1 px-3 text-neutral-400 dark:text-neutral-500">
                    Order by
                  </div>
                </li>
        ...
        <template v-if="parentFolder || viewContents.folders || viewContents.files">
          <table class="table mb-4">
            ...
            <thead>
              <th
                v-for="field in view.config.fields"
                :class="[
                  field == view.config.primary ? 'primary-field' : '',
                  `field-type-${fieldsSchemas[field]?.type}`,
                ]"
              >
                {{ fieldsSchemas[field]?.label }}
              </th>
              <th class="actions">&nbsp;</th>
            </thead>
            <tbody>
              ...
              <!-- Subfolders -->
              <tr
                v-for="item in viewContents.folders"
                :key="item.name"
                v-if="schema.subfolders !== false"
                class="h-[47px]"
              >
                <td colspan="100%" class="folder">
                  <router-link
                    :to="{ name: $route.name, query: { ...$route.query, folder: item.path } }"
              ...
              <tr v-for="item in viewContents.files" :key="item.filename">
                ...
                <td class="actions text-right">
                  <div class="inline-flex">
                    <router-link
                      :to="{ name: 'edit', params: { name: name, path: item.path } }"
                      class="btn-sm !border-r-0 !rounded-r-none"
                      >Edit</router-link
                    >
                    <Dropdown :dropdownClass="'!max-w-none w-48'">
                      <template #trigger>
                        <button
                          class="btn-icon-sm group-[.dropdown-active]:bg-neutral-100 dark:group-[.dropdown-active]:bg-neutral-850 !rounded-l-none"
                        >
                          <Icon name="MoreVertical" class="h-4 w-4 stroke-2 shrink-0" />
                        </button>
                      </template>
                      <template #content>
                        <ul>
                          ...
                          <li>
                            <hr class="border-t border-neutral-150 dark:border-neutral-750 my-1" />
                          </li>
                          <li>
                            <button class="link w-full" @click="openRenameModal(item)">
                              Rename
                            </button>
                          </li>
                          <li>
                            <router-link
                              :to="{ name: 'new', params: { path: item.path } }"
                              class="link w-full"
                              >Make a copy</router-link
                            >
                          </li>
                          <li>
                            <button class="link-danger w-full" @click="openDeleteModal(item)">
                              Delete
                            </button>
                          </li>
                        </ul>
        ...
        >
          <div class="max-w-md mx-auto">
            ...
            <p class="text-neutral-400 dark:text-neutral-500">
              There are no entries yet for the "{{ schema.label || schema.name }}" collection here.
            </p>
          </div>
          <div class="flex gap-x-2 justify-center mt-4">
            <router-link :to="{ name: 'new', query: { folder: folder } }" class="btn-primary-sm"
              >Add an entry</router-link
            >
          </div>
        </div>
        <!-- Empty search -->
        <div
          v-else-if="viewContents.files.length == 0"
          class="text-center rounded-xl bg-neutral-100 dark:bg-neutral-850 p-6"
        >
          <div class="max-w-md mx-auto">
            <h2 class="font-semibold tracking-tight">No results</h2>
            <p class="text-neutral-400 dark:text-neutral-500">
              There are no results for your search terms. You can use wildcard (e.g. "keyword*") or
              field specific search (e.g. "title:keyword").
            </p>
            <div class="flex gap-x-2 justify-center mt-4">
...
const schemaFields = computed(() => {
  let fieldsArray = schema.value.fields
...
const parentFolder = computed(() => {
  ...
  const query = {
    ...route.query,
    folder: parentPath !== schema.value.path ? parentPath : undefined,
  };
  return {
    name: route.name,
    query: query,
...
const setView = () => {
  view.config.primary =
    (schema.value.view && schema.value.view.primary) ||
    (fields.value.includes("title") ? "title" : fields.value[0]);
  view.config.fields =
    (schema.value.view && schema.value.view.fields) ||
    (collection.value[0]?.fields?.date ? [view.config.primary, "date"] : [view.config.primary]);
  view.config.sort =
    (schema.value.view && schema.value.view.sort) ||
    (collection.value[0]?.fields?.date ? ["date", view.config.primary] : [view.config.primary]);
  view.sort =
    (schema.value.view && schema.value.view.default && schema.value.view.default.sort) ||
    (view.config.sort && view.config.sort.length)
      ? view.config.sort[0]
      : null;
  view.order =
    (schema.value.view && schema.value.view.default && schema.value.view.default.order) || "desc";
  view.search =
    (schema.value.view && schema.value.view.default && schema.value.view.default.search) || "";
};
...
const setCollection = async () => {
  ...
  collection.value = files
    ...
    .map((file) => {
      if (
        file.type === "blob" &&
        (extension.value === "" || file.name.endsWith(`.${extension.value}`))
      ) {
        let contentObject = {};
  ...
  if (errorCount > 0) {
    ...
    notifications.notify(
      `Failed to parse frontmatter for ${errorCount} ${
        errorCount > 1 ? "entries" : "entry"
      }. Your settings may be wrong.`,
      "error",
      options
    );
  }

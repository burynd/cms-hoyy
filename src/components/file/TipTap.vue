        ...
        <template v-if="!isCodeEditor">
          ...
          <button
            v-if="
              ...
              (props.options?.input !== undefined && props.options?.output !== undefined)
            "
...
const insertImage = async () => {
  if (imageSelection.value.length) {
    imageSelection.value.forEach(async (selectedImage) => {
      ...
      console.log("Image inserted: " + rawUrl);
    });
  }
...
const exportContent = (content) => {
  ...
  let htmlContent = content.replace(
    /src="\/files\//g,
    `src="https://raw.githubusercontent.com/${repoStore.owner}/${repoStore.repo}/${repoStore.branch}/files/`
  );
  return props.format === "markdown" ? turndownService.turndown(htmlContent) : htmlContent;
};
        ...
        for (const file of files) {
          if (allowedTypes.includes(file.type)) {
            ...
            notifications.notify(
              "Only images can be uploaded (JPEG, PNG, GIF, SVG, TIFF and BMP).",
              "error"
            );
          }
        }
...
const upload = async (file) => {
  if (file) {
    ...
    if (content) {
      const notificationId = notifications.notify(`Uploading "${file.name}".`, "processing", {
        delay: 0,
      });

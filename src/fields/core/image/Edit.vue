<template>
  ...
  >
    <template #item="{ element, index }">
      <li v-if="element" class="relative w-full cursor-move">
        ...
        <div class="absolute bottom-0 right-0 z-10 flex p-2">
          ...
          <button class="btn-icon-sm relative group" @click="downloadImage(element)">
            <Icon name="Download" class="h-4 w-4 stroke-2 shrink-0" />
            <div class="tooltip-top">Download image</div>
          </button>
    ...
    <template #footer>
      <li
        v-if="
          internalModelValue.length < (props.field.list ? props.field.list?.max ?? Infinity : 1)
        "
      >
        <button
  ...
  <Modal ref="selectImageModal" :componentClass="'modal-file-browser'">
    ...
    <template #content>
      ...
      <footer class="flex justify-end text-sm gap-x-2 mt-4">
        ...
        <button class="btn-primary" @click="confirmImageSelection()">Confirm</button>
      </footer>
    </template>
  </Modal>
</template>
...
const confirmImageSelection = () => {
  if (imageSelection.value.length > 0) {
    const insertIndex =
      activeImgIndex.value !== null ? activeImgIndex.value : internalModelValue.value.length;
    internalModelValue.value.splice(
      insertIndex,
...
const setImages = () => {
  if (props.modelValue) {
    ...
    internalModelValue.value.forEach((imagePath, index) => {
      ...
      internalModelValue.value[index] = githubImg.swapPrefix(
        imagePath,
        prefixOutput.value,
        prefixInput.value,
        true
...
watch(
  ...
  (newValue) => {
    if (newValue) {
      ...
      const modelValueOutput = newValue.map((path) =>
        githubImg.swapPrefix(path, prefixInput.value, prefixOutput.value)
      );
      if (props.field.list) {

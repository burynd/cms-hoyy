    ...
    validation = errors.map((error) => {
      const yamlPath = error.instancePath
        ? error.instancePath
            .substring(1)
            .split("/")
            .map((segment) => (isNaN(segment) ? segment : parseInt(segment, 10)))
        : [];
      ...
      if (error.keyword === "additionalProperties") {
        ...
        const parentNode =
          yamlNode &&
          yamlNode.items &&
          yamlNode.items.find((item) => item.key.value === error.params.additionalProperty);
...
const set = async (owner, repo, branch) => {
  ...
  if (!entry || entry.sha !== file.sha) {
    ...
    if (entry.content.trim() === "") {
      ...
      ({ document: entry.document, validation: entry.validation } = parse(
        owner,
        repo,
        branch,
        entry.content
      ...
      if (entry.object?.content?.length > 0) {
        entry.object.content = entry.object.content.map((item) => {
          ...
          if (item.filename == null && item.type === "collection") {
            item.filename = "{year}-{month}-{day}-{primary}.mdx"
              ? "{year}-{month}-{day}-{primary}.mdx"
